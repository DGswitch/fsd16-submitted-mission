        ...
        if (script?.textContent?.length > 0) {
            for (const match of script.textContent.matchAll(regEx)) {
                ...
                if (match[1] === 'init') { // ("init", "pixelId', optional something else")
                    const currentPixelId = match[2].match(/['"]?([\d]+)['"]?/)? match[2].match(/['"]?([\d]+)['"]?/)[1] : match[2];
                ...
                } else if (pixelIds.length > 0) { // ("track", "pixelEventName', optional something else")
                    const pixelEvent = match[2].match(/['"]?([\w]+)['"]?/)? match[2].match(/['"]?([\w]+)['"]?/)[1] : match[2];
...
function parseNoScriptEvents(noScriptElements, frame, parsedEvents) {
    for (const noscript of noScriptElements) {
        const match = noscript?.textContent?.match(/src[\s*]?=[\s*]?['"]https:\/\/www\.facebook\.com\/tr[\?]?id=([\d]*)(&amp;|&)ev=([^&]*)[&'"]/);
        if (match) {
            ...
            parsedEvents[match[1]][match[3]]["pixelCode"] = noscript.outerHTML.replace(/[\u00A0-\u9999<>\&]/gim,
...
function parsedImgEvents(imgElements, frame, parsedEvents) {
    for (const img of imgElements) {
        ...
        if (match) {
            ...
            // Encode HTML element, https://stackoverflow.com/questions/18749591/encode-html-entities-in-javascript
            parsedEvents[match[1]][match[3]]["pixelCode"] = img.outerHTML.replace(/[\u00A0-\u9999<>\&]/gim,
                    function(i) {  return '&#' + i.charCodeAt(0) + ';';});;
            parsedEvents[match[1]][match[3]]["frame"] = frame;
        }
    }
...
function parseContent() {
    ...
    const iframes =
        Array.prototype.slice.call(window.parent.frames).map(window => {
        try {
