...
const piiDataMatchRule = /ud\[([^\]]+)\]/; // ud[balabala]='1234'
...
function updateDiagnosticsStatus(pixelEventName, pixelEventAttribute) {
  if (pixelEventName.startsWith("Microdata") || pixelEventName.startsWith("SubscribedButtonClick")) {
    pixelEventAttribute["status"] = "auto";
  } else if (!pixelEventAttribute["urlCalled"]) { // no pixel request fired
    ...
    +   "</a></li><ol>";
  } else if (pixelEventAttribute["statusType"] === "fired_multiple_times") { // multiple reqeusts fired
    pixelEventAttribute["status"] = "warning";
    pixelEventAttribute["statusContent"] = "<span>The Facebook pixel activated " + pixelEventAttribute["statusCount"] + " times on this web page, which can cause errors in your event tracking.</span>"
...
function fillInTrDetails(request, eventParams, tabId) {
  ...
  for (const key of eventParams.keys()) {
    ...
    if (matchedPiiData) {
      piiData[matchedPiiData[1]] = eventParams.get(matchedPiiData[0]);
    }
    if (matchedDpoData) {
  ...
  if (trDetails[tabId][pixelId][pixelEvent]) {
    // Check duplication
    if (JSON.stringify(trDetails[tabId][pixelId][pixelEvent]["customData"]) == JSON.stringify(customData)) {
      trDetails[tabId][pixelId][pixelEvent]["statusType"] = "fired_multiple_times";
      trDetails[tabId][pixelId][pixelEvent]["statusCount"] = trDetails[tabId][pixelId][pixelEvent]["statusCount"] ?
          trDetails[tabId][pixelId][pixelEvent]["statusCount"] + 1 : 2;
    } else {
      ...
      if (parsedContent && Object.entries(parsedContent).length > 0) {
        ...
        for (const pixelId in parsedContent) {
          ...
          for (const pixelEvent in pixelEvents) {
            ...
            parsedDetails[tabId][pixelId][pixelEvent]["pixelCode"] = pixelEvents[pixelEvent]["pixelCode"];
