function displayOverview(pixelSize, tabURL) {
  ...
  if (pixelSize == 0) {
    overview.innerHTML = "No pixel found on " + tabURL
...
function bindPopoutWindow() {
  ...
  popOutButton[0].addEventListener("click",
    function(event) {
...
function bindEventListener() {
  ...
  for (let i = 0; i < pixelEventsActions.length; i++) {
    pixelEventsActions[i].addEventListener("click", function() {
  ...
  for (let i = 0; i < stringActions.length; i ++) {
    stringActions[i].addEventListener("click", function() {
      hideShowString(this);
    }, false);
  }
  ...
  for (let i = 0; i < copyActions.length; i ++) {
    copyActions[i].addEventListener("click", function() {
      clickToCopy(this);
    }, false);
  }
}
...
function displayPixelEventsDetails(pixelEventName, pixelEventInfo) {
  ...
  if (pixelEventInfo["customData"] && Object.entries(pixelEventInfo["customData"]).length > 0) {
    ...
    for (const customDataDetail in pixelEventInfo["customData"]) {
      let customDetailDetailAttribute = pixelEventInfo["customData"][customDataDetail] ? pixelEventInfo["customData"][customDataDetail] : "<em>Not Set</em>";
  ...
  if (pixelEventInfo["dpoData"] && Object.entries(pixelEventInfo["dpoData"]).length > 0) {
    ...
    eventDetails += `<span style="display: inline-block; margin-top: 8px;">`
  ...
  if (pixelEventInfo["status"] === "warning") {
     eventDetails += "<div><div class='eventInfoTitle'><strong>WARNINGS</strong></div>"
  ...
  if (pixelEventInfo["frame"]) { // TODO: add frame
...
function displayPixelEventsInfoDetails(pixelEventName, pixelEventDetails) {
  ...
  if (pixelEventDisplayLabel === 'Microdata' || pixelEventDisplayLabel === 'SubscribedButtonClick') {
    imgAlt = "<img alt=\"We've suggested this event category based on the button text and user activity on your website.\" "
      ...
      + "class='autoDetect'>";
    pixelEventDisplayLabel = (pixelEventDisplayLabel === "Microdata")? "Microdata Automatically Detected" : "Button Click Automatically Detected";
  }
  ...
  let pixelEventsDetail
      = "<div>"
        ...
        +     "<img src='/img/" + pixelEventDetails["status"] + "_icon.png' class='pixelEventIconMoreInfo'>"
        +     "<span>" + pixelEventDisplayLabel + "</span>"
        +       imgAlt
        +   "</p>"
        + "</div>"
...
function displayPixelInfo(pixelId, pixelEvents, tabURL) {
  const troubleshootLink = "https://www.facebook.com/events_manager2/list/pixel/" + pixelId + "/test_events";
  const setupEventLink = "https://www.facebook.com/events_manager2/list/pixel/" + pixelId + "/settings";

  let pixelInfo =
        "<div class='pixelPanelOverviewTitle'>"
    +     "<span>Meta Pixel</span>"
    +       "<div class='pixelPanelOverviewLinks'>" // Links for each pixel
    +         "<a href=" + troubleshootLink + " class='troubleShootingLink'>Troubleshoot Pixel</a>"
    +         "<div class='setupEventsContainer'><a href="+ setupEventLink +" class='setupEventsLink'><span>Set up events</span></a></div>"
    +       "</div>"
    +   "</div>"
    +   "<p class='pixelIdDisplay'><span style='cursor:pointer;'>"
    +     "<span text="+ pixelId + ">Pixel ID: " + pixelId + "</span>&nbsp;"
